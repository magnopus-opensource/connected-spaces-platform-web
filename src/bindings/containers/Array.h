...
emscripten::register_type<bindings::utils::JSDisposable<csp::common::Array<StorageType>>>("(StorageType[] & Disposable)");
...
emscripten::class_<TypeToBind>("TypeToBind")
    ...
    .function("functionThatReturnsArray", +[](const TypeToBind& self) {
        return bindings::utils::JSDisposable<csp::common::Array<StorageType>>{self.FunctionThatReturnsArray()};
    })
 ...
 *  emscripten::register_type<bindings::utils::JSDisposable<csp::common::Array<MyType>>>("(MyType[] & Disposable)");
 */
namespace emscripten::internal {

template <typename T> struct BindingType<csp::common::Array<T>> {
    using ValBinding = BindingType<val>;
    using WireType = ValBinding::WireType;

    // Parameter-path only: no [Symbol.dispose] attached. Returns go through
    // bindings::utils::JSDisposable<csp::common::Array<T>> instead.
    static WireType toWireType(const csp::common::Array<T>& arr, rvp::default_tag)
    {
        ...
        for (size_t i = 0; i < arr.Size(); ++i) {
            if constexpr (std::is_pointer_v<T>) {
                ...
                newJSArray.set(i, bindings::utils::NonOwningValRef(arr[i]));
            } else {
...
template <typename T> struct BindingType<bindings::utils::JSDisposable<csp::common::Array<T>>> {
    using ValBinding = BindingType<val>;
    using WireType = ValBinding::WireType;
